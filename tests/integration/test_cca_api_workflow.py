...
def test_act_7_nexus_clinical_brief_and_mdt_package(client, headers, db_session, doctor):
    ...
    # architecture doc Sec 20's 7th required brief component) plus a 15th Radiation Therapy
    # Summary section (Oncology Review Results PDF item 5 -- radiation course/phase/fraction
    # data pulled automatically, never re-typed).
    ...
    assert "15_radiation_therapy" in sections
...
def test_document_upload_is_restricted_to_front_desk_roles(client, headers, auth_headers, make_user, db_session, doctor, monkeypatch):
    """Document upload must exist only in Front Desk -- everywhere else in the app, not just the
    UI. Nurse Navigator (and every other CCA persona) can still read documents Front Desk
    already uploaded (list_documents has no such gate), but must never be able to upload one
    itself, whether through the UI or a direct API call."""
    from app.routers import cca as cca_router

    patient_id = _demo_patient_id(db_session, doctor.organization_id)
    nurse_navigator = make_user(
    ...
    denied = client.post(
        f"/api/cca/documents?patient_id={patient_id}",
        files={"file": ("x.pdf", b"%PDF-1.4 x", "application/pdf")},
        headers=auth_headers(nurse_navigator),
    )
    assert denied.status_code == 403

    # Nurse Navigator can still read the document list -- no read-side gate was ever intended.
    listing = client.get(f"/api/cca/documents?patient_id={patient_id}", headers=auth_headers(nurse_navigator))
    assert listing.status_code == 200

    # Front Desk (and Doctor, per `headers`) is unaffected by the new gate.
    allowed = client.post(
        f"/api/cca/documents?patient_id={patient_id}",
        files={"file": ("y.pdf", b"%PDF-1.4 y", "application/pdf")},
        headers=headers,
    )
    assert allowed.status_code == 201

    front_desk = make_user(
        email="frontdesk@ccahosp.com", role="CCAFrontDesk", organization_id=doctor.organization_id,
    )
    allowed_front_desk = client.post(
        f"/api/cca/documents?patient_id={patient_id}",
