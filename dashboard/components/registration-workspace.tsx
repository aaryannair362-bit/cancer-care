...
function IdentityDocumentCard({
  ...
  return <div className="aivana-gradient-soft rounded-xl border border-white/70 p-4 shadow-soft-sm">
    ...
    <div className="mt-4 grid gap-4 sm:grid-cols-2">
      <label className="text-xs font-medium text-metadata">{isAadhaar ? 'Aadhaar number' : 'Ration card number'}<Input className="mt-1" inputMode={isAadhaar ? 'numeric' : 'text'} placeholder={isAadhaar ? 'Enter Aadhaar number' : 'Enter ration card number'} /></label>
      <label className="text-xs font-medium text-metadata">{isAadhaar ? 'Name as per Aadhaar' : 'Name as per ration card'}<Input className="mt-1" placeholder="Enter name on card" /></label>
      ...
      <label className="flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-md border border-dashed border-border bg-surface/70 px-3 py-2 text-xs font-medium text-supporting transition-colors hover:bg-surface sm:col-span-2"><Upload className="size-4 text-metadata" aria-hidden="true" />Upload {title}<input type="file" accept="image/*,.pdf" className="sr-only" /></label>
    </div>
  </div>
}
...
export function RegistrationWorkspace() {
  ...
  return <PageContainer>
    ...
    <div className="grid gap-6 xl:grid-cols-3"><div className="space-y-6 xl:col-span-2">
      ...
      <Card><CardHeader className="border-b border-divider"><CardTitle>Documents</CardTitle><CardDescription>Attachment state only; no files are uploaded or transmitted</CardDescription></CardHeader><CardContent className="space-y-3 pt-6">{documentLabels.map((item)=><div key={item} className="flex items-center justify-between gap-3 rounded-md border border-border p-3"><div className="flex items-center gap-3"><FileText className="size-4 text-metadata"/><div><p className="text-sm font-medium">{item}</p><p className="text-xs text-metadata">{item.replace(/ /g,'_')}_DEMO.pdf</p></div></div><Button type="button" size="sm" variant="outline" onClick={()=>setDocuments((value)=>({...value,[item]:true}))}>{documents[item]?'Attached':'Mark demo attached'}</Button></div>)}</CardContent></Card>
